function out = pool(varargin)
% POOL MATLAB Pool Demonstration
%
% Husam Aldahiyat, numandina@gmail.com, 2011
%
%
% out = pool([ex ey],[angle power], [sx sy])
%  - [ex ey] = location of the enemy's ball
%  - [angle power] = angle and power of shot. Note, power should be around
%  the range of 3000-12000, and angle should be also between -40000 and
%  40000 just to give an idea
%  - [sx sy] is the starting position of the cue ball.
%
...
if(length(varargin) == 3)
  ...
  disp('To run trial: pool([g_x g_y], [angle force]');
  single = false;
  out = [];
end
...
if(single)
  h1 = movsph(1,varargin{3},varargin{3},25,0,[],0);
else
...
hold on
  ...
  function go1(varargin)
    ...
    if length(varargin) == 1
      h1 = movsphm(1,str2num(varargin{1}),h1);
    else
      h1 = movsphm(1,pt,h1);
    end
    chs
