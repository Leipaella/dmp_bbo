function [split_decision split_feature split_value] = feature_split_sliding(percepts, costs, p_thresh, fig,test)
%
% For each feature: choose possible split value in between each of the
% given feature values. Then use this split value to group the feature-cost
% pairs into two groups (below the split_val and above the split_val). See
% if these two groups have significantly different cost distributions with
% the Student T-test. (Forcing seperability, seeking distinguishability)
%
% Inputs:
...
for i_feature = 1:n_features
  ...
  %------------------------------------------------------------------------------
  % Continuous features - need to run through possible split values to find
  % the best one
  else
    fsorted = sort(f);
    fsorted = fsorted(min_group_size:end-min_group_size);
    possible_splits = (fsorted(2:end) + fsorted(1:end-1))/2;
    
    for i_split = 1:length(possible_splits)
      val = possible_splits(i_split);
      f1 = f(f<val);
      f2 = f(f>val);
      c1 = c(f<val);
      c2 = c(f>val);
      ...
      if t>0, t = -t; end
      p = 2*tcdf(t,v); %two tailed test
      p_arr_splits(i_split) = p;
    end
    [pmin imin] = min(p_arr_splits);
    p_arr(i_feature) = pmin;
    split_val(i_feature) = possible_splits(imin);
    
    

    %------------------------------------------------------------------------------
    % Plot a line where the highest probability split is located, and show
  ...
  for c_i = 1:size(c,1)
    for f_i = 1:size(f,1)
      for f_rel_i = 1:size(f_rel,1)
        ...
        feature = f(f_i,:);
        cost = c(c_i,ordering);
        [split_decision split_feature split_value] = feature_split_sliding(feature',cost',0.05,1,1);
